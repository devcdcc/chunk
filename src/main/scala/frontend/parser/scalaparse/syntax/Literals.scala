...
trait Literals { l =>
  ...
  /**
   * Parses all whitespace, excluding newlines. This is only really useful in
   * e.g. {} blocks, where we want to avoid capturing newlines so
   * semicolon-inference would work
   */
  ...
  object Literals {
    ...
    class InterpCtx(interp: Option[() => P[Unit]]) {
      ...
      /**
       * Helper to quickly gobble up large chunks of un-interesting characters.
       * We break out conservatively, even if we don't know it's a "real" escape
       * sequence: worst come to worst it turns out to be a dud and we go back
       * into a CharsChunk next rep
       */
