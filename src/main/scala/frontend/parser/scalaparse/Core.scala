package chunk
package frontend.parser.scalaparse
...
trait Core extends syntax.Literals {
  ...
  implicit class TrailingCommaOps[+T](p0: => P[T]) {
    def repTC[R](min: Int = 0, max: Int = Int.MaxValue, exactly: Int = -1)(implicit
      ev: fastparse.Implicits.Repeater[T, R],
      ctx: P[_]
    ): P[R] =
  ...
  /**
   * Sketchy way to whitelist a few suffixes that come after a . select; apart
   * from these and IDs, everything else is illegal
   */
